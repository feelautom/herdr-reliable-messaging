...
Claude also proposes a complete next message inside its own composer, rendered dimmed and accepted with Tab. The detection source strips styling, so that suggestion is indistinguishable from exact user text and would block delivery forever. When a Claude composer appears to hold unknown content and no chunk has been loaded yet, the dispatcher performs one additional styled read through `agent read --source visible --ansi`. If every visible character after the marker carries SGR 2, the content belongs to the agent and the composer counts as free. Text typed by the user carries no styling at all and stays protected exactly as before. A failed or mixed styled read keeps the transaction pending without writing.
